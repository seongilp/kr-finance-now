...
export function DatasetSearch({ index }: { index: DatasetSummary[] }) {
  ...
  const hits = useMemo(() => {
    ...
    return index.filter((d) => terms.every((t) => `${d.title} ${d.desc}`.toLowerCase().includes(t))).slice(0, MAX_RESULTS);
  }, [deferred, index]);
    ...
    <div>
      ...
      <div className="relative">
        ...
        <Input
          ...
          placeholder={`${index.length.toLocaleString()}개 데이터에서 찾기 — 예: 주식시세, 배당, 펀드, 실손, 저축은행`}
      ...
      {deferred.trim() && (
        <ul className="mt-2 divide-y rounded-lg border bg-card">
          ...
          {hits.map((d) => (
            <li key={d.id}>
              <Link href={`/d/${d.id}`} className="block p-3 hover:bg-accent">
